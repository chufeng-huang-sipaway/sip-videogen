"""User preferences management for sip-studio.
This module handles persistent user preferences stored in
~/.sip-studio/config.json, allowing users to configure
...
class SoraPreferences(BaseModel):
    """Sora-specific user preferences."""
...
class UserPreferences(BaseModel):
    """User preferences stored in ~/.sip-studio/config.json."""

    # Sip Studio also stores API keys and other settings in the same config file.
    ...
    sora: SoraPreferences = Field(
        default_factory=SoraPreferences,
        description="Sora-specific preferences",
    )

    @classmethod
    def get_config_dir(cls) -> Path:
        """Get the config directory path."""
        return Path.home() / ".sip-studio"
    ...
    def save(self) -> None:
        ...
        # Merge into existing config to avoid overwriting Sip Studio settings
