...
<div className="grid gap-4">{/* Constraints */}<div><label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Brand Constraints</label><p className="text-xs text-muted-foreground mb-2">Rules and requirements the brand must always follow</p><div className="mt-1 flex flex-wrap gap-2">{data.constraints.length>0?(data.constraints.map((constraint,index)=>(<span key={index} className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-muted text-muted-foreground">{constraint}</span>))):(<span className="text-sm text-muted-foreground italic">No constraints defined</span>)}</div></div>
{/* Avoid */}<div><label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Things to Avoid</label><p className="text-xs text-muted-foreground mb-2">Content, messaging, or approaches the brand should never use</p><div className="mt-1 flex flex-wrap gap-2">{data.avoid.length>0?(data.avoid.map((item,index)=>(<span key={index} className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-brand-a10 text-brand-600 dark:text-brand-500">{item}</span>))):(<span className="text-sm text-muted-foreground italic">No items to avoid defined</span>)}</div></div></div></div>)
//Edit mode content
