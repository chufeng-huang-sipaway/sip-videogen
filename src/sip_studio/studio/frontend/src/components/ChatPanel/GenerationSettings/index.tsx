        ...
        <Popover open={open} onOpenChange={setOpen}>
            <PopoverTrigger asChild>
                <button
                    ...
                    disabled={disabled}
                    className={cn(
                        "flex items-center gap-1.5 h-8 px-2.5 rounded-full text-xs font-medium transition-all bg-white/50 dark:bg-white/10 border border-border/40",
                        "text-muted-foreground hover:text-foreground hover:bg-white/80 dark:hover:bg-white/20",
                        disabled && "opacity-50 cursor-not-allowed"
                    )}
                    title="Generation defaults"
                >
                    <RatioIcon ratio={imageAspectRatio} size={12} className="opacity-80" />
                    <span className="mx-0.5 text-muted-foreground/30">/</span>
                    <RatioIcon ratio={videoAspectRatio} size={12} className="opacity-80" />
