    ...
    >
      ...
      <div className={panelMode === 'assistant' ? 'flex flex-col flex-1 min-h-0' : 'hidden'}>
        ...
        <div className="px-4 pb-6 pt-2 w-full max-w-3xl mx-auto z-20">
          <MessageInput ref={messageInputRef} disabled={isLoading || !brandSlug} isGenerating={isLoading} onCancel={cancelGeneration} placeholder="" onMessageChange={setInputText} webSearchEnabled={webSearchEnabled} deepResearchEnabled={deepResearchEnabled} onWebSearchToggle={() => setWebSearchEnabled(!webSearchEnabled)} onDeepResearchToggle={() => setDeepResearchEnabled(!deepResearchEnabled)} onSend={async (text) => { const mentionAtts = resolveMentions(text, products, styleReferences); const allProducts = [...new Set([...attachedProducts, ...mentionAtts.products])]; const srMap = new Map<string, AttachedStyleReference>(); for (const t of mentionAtts.styleReferences) srMap.set(t.style_reference_slug, t); for (const t of attachedStyleReferences) srMap.set(t.style_reference_slug, t); const allStyleRefs = Array.from(srMap.values()); await sendMessage(text, { project_slug: activeProject, attached_products: allProducts.length > 0 ? allProducts : undefined, attached_style_references: allStyleRefs.length > 0 ? allStyleRefs : undefined, image_aspect_ratio: imageAspectRatio, video_aspect_ratio: videoAspectRatio, web_search_enabled: webSearchEnabled, deep_research_enabled: deepResearchEnabled }); for (const slug of mentionAtts.products) { if (!attachedProducts.includes(slug)) attachProduct(slug) } for (const sr of mentionAtts.styleReferences) { if (!attachedStyleReferences.some(t => t.style_reference_slug === sr.style_reference_slug)) attachStyleReference(sr.style_reference_slug, sr.strict) } await refreshProducts() }} canSendWithoutText={attachments.length > 0} onSelectImages={handleSelectImages} hasProducts={products.length > 0} hasStyleReferences={styleReferences.length > 0} />
        </div>
      </div>
