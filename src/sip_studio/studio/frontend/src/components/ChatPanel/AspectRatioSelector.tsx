...
function ImageRatioDropdown({ value, onChange, disabled, allowedRatios }: { value: AspectRatio, onChange: (r: AspectRatio) => void, disabled: boolean, allowedRatios?: AspectRatio[] }) {
    ...
    return (
        <DropdownMenu>
            ...
            <DropdownMenuContent align="start" side="top" sideOffset={8} className="w-56 p-1.5 shadow-lg border-border/50">
                {ratios.map(r => {
                    const { hint } = ASPECT_RATIOS[r]
                    const sel = value === r
                    return (<DropdownMenuItem key={r} onClick={() => onChange(r)} className={cn("flex items-center gap-3 px-3 py-2 rounded-lg cursor-pointer", sel ? "bg-primary/5 text-primary" : "text-muted-foreground hover:bg-secondary/50 hover:text-foreground")}>
