...
function SubSectionGroup({ title, icon, count, isOpen, onToggle, onAdd, activeBrand, children }: any) { return (<div className="group/subsection pl-2"><div className="flex items-center justify-between py-1 pr-2 mb-0.5"><button onClick={onToggle} className="flex-1 flex items-center gap-2.5 py-1.5 px-3 rounded-full text-xs font-semibold text-muted-foreground/80 hover:text-foreground hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-all duration-200 text-left active:scale-95 active:shadow-inner" disabled={!activeBrand}><div className={cn("transition-transform duration-200", isOpen ? "rotate-0" : "-rotate-90 text-muted-foreground/50")}><ChevronDown className="w-3 h-3" strokeWidth={1.5} /></div>{icon && <span className="text-muted-foreground/60 group-hover/subsection:text-foreground/70 transition-colors">{icon}</span>}{title}{count !== undefined && <span className="ml-0.5 text-[10px] font-normal opacity-60">({count})</span>}</button><Button variant="ghost" size="icon" className="h-6 w-6 rounded-full hover:bg-white dark:hover:bg-white/10 hover:shadow-sm opacity-0 group-hover/subsection:opacity-100 transition-all duration-200" onClick={(e) => { e.stopPropagation(); onAdd?.(); }} disabled={!activeBrand}><Plus className="w-3.5 h-3.5" strokeWidth={1.5} /></Button></div><div className={cn("grid transition-all duration-150 ease-in-out border-l border-border/10 ml-[18px] pl-2", isOpen ? "grid-rows-[1fr] opacity-100 py-1" : "grid-rows-[0fr] opacity-0")}><div className="overflow-hidden">{children}</div></div></div>) }
