...
export function ThumbnailStrip() {
    ...
    return (
        <div className="flex-shrink-0 w-full px-8 pb-4 absolute bottom-0 z-20 pointer-events-none flex justify-center">
            <div className="flex gap-2 overflow-x-auto scrollbar-hide justify-center px-4 py-3 bg-white/20 dark:bg-black/20 backdrop-blur-xl border border-white/10 shadow-lg rounded-2xl pointer-events-auto max-w-[80vw]">
                {currentBatch.map((img, i) => {
                    const imgPath = img.originalPath || img.path || '';
                    const canDrag = !!imgPath && !imgPath.startsWith('data:') && !isGenerating;
                    return (
                        <button
                            key={img.id}
                            ...
                            onClick={() => !isGenerating && setSelectedIndex(i)}
                            disabled={isGenerating}
                            className={cn(
                                "flex-shrink-0 w-10 h-10 rounded-lg overflow-hidden transition-all duration-300 relative",
                                isGenerating ? "cursor-not-allowed opacity-50" : "cursor-grab active:cursor-grabbing",
                                i === selectedIndex
                                    ? "ring-2 ring-brand-500 shadow-md scale-110 z-10"
