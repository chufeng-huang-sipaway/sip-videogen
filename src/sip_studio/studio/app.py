...
def _set_macos_app_name() -> None:
    """Set app name in macOS dock/menu bar for development mode."""
    if sys.platform != "darwin" or is_bundled_app():
        return
    try:
        ...
        info["CFBundleName"] = "Sip Studio"
        # Set process name
        NSProcessInfo.processInfo().setProcessName_("Sip Studio")
        # Try to update dock tile
        app = NSApplication.sharedApplication()
        app.setActivationPolicy_(0)  # NSApplicationActivationPolicyRegular
        logger.debug("Set macOS app name to 'Sip Studio'")
...
def get_frontend_url() -> str:
    ...
    if is_bundled_app():
        ...
        # __file__ is typically: .app/Contents/Resources/lib/python3.x/sip_studio/studio/app.py
...
def main():
    """Launch the Sip Studio application."""
    # Set app name in dock/menu bar for dev mode
    _set_macos_app_name()
    # Run data migration if needed
    from sip_studio.config.migration import migrate_if_needed

    migrate_if_needed()
    # Set up logging with file output for debugging
    from sip_studio.config.logging import setup_logging

    log_file = Path.home() / ".sip-studio" / "studio.log"
    setup_logging(level="INFO", log_file=log_file)
    logging.getLogger("sip_studio").info(f"=== Sip Studio started, logging to {log_file} ===")
    try:
        import webview
    except ImportError:
