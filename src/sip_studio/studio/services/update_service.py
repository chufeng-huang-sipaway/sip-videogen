...
from sip_studio.studio import __version__
from sip_studio.studio.updater import (
    DownloadProgress,
    UpdateInfo,
    download_update,
    get_current_version,
...
class UpdateService:
    ...
    def check_for_updates(self) -> dict:
        ...
        try:
            ...
            if update_info is None:
                return bridge_ok({"has_update": False, "current_version": get_current_version()})
            return bridge_ok(
                {
                    "has_update": True,
    ...
    def download_and_install_update(self, download_url: str, version: str) -> dict:
        ...
        try:
            ...
            if not dmg_path or not dmg_path.exists():
                self._state.update_progress = {"status": "error", "error": "Download failed"}
                return bridge_error("Failed to download update")
            self._state.update_progress = {"status": "installing", "percent": 100}
            if not install_update(dmg_path):
                self._state.update_progress = {"status": "error", "error": "Install failed"}
                return bridge_error("Failed to start update installation")
            ...
            self._state.update_progress = {"status": "error", "error": str(e)}
            return bridge_error(str(e))
    ...
    def get_update_progress(self) -> dict:
        ...
        if p is None:
            return bridge_ok({"status": "idle", "percent": 0})
        return bridge_ok(p)
    ...
    def get_update_settings(self) -> dict:
        ...
        try:
            return bridge_ok(get_update_settings())
        except Exception as e:
            return bridge_error(str(e))
