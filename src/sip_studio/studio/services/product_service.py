...
class ProductService:
    ...
    def get_product(self, product_slug: str) -> dict:
        ...
        try:
            ...
            if not product:
                return bridge_error(f"Product '{product_slug}' not found")
            description = merge_attributes_into_description(
                product.description or "", product.attributes
            )
            return bridge_ok(
                {
                    "slug": product.slug,
                    ...
                    "packaging_text": product.packaging_text.model_dump(mode="json")
    ...
    def create_product(
        ...
        try:
            ...
            if not slug:
                return bridge_error("No brand selected")
            if not name.strip():
                return bridge_error("Product name is required")
            product_slug = re.sub(r"[^a-z0-9]+", "-", name.lower()).strip("-")
            if not product_slug:
                return bridge_error("Invalid product name")
            if load_product(slug, product_slug):
                return bridge_error(f"Product '{product_slug}' already exists")
            ...
            description_text = merge_attributes_into_description(
                description_text, parsed_attributes
            )
            now = datetime.utcnow()
            product = ProductFull(
                slug=product_slug,
            ...
            if images:
                for img in images:
                    ...
                    try:
                        ...
                        logger.warning("Failed to add product image %s: %s", filename, e)
    ...
    def update_product(
        ...
        try:
            ...
            product.description = merge_attributes_into_description(
                description_text, product.attributes
            )
    ...
    def set_primary_product_image(self, product_slug: str, filename: str) -> dict:
        ...
        try:
            ...
            if brand_relative_path not in product.images:
                return bridge_error(f"Image '{filename}' not found in product")
    ...
    async def analyze_all_packaging_text(self, skip_existing: bool = True) -> dict:
        ...
        try:
            ...
            result = await _impl_analyze_all_product_packaging(
                skip_existing=skip_existing, skip_human_edited=True, max_products=50
            )
