...
class ProjectService:
    ...
    def get_project(self,project_slug:str)->dict:
        ...
        try:
            ...
            if not project:return bridge_error(f"Project '{project_slug}' not found")
            assets=list_project_assets(slug,project_slug)
            return bridge_ok({"slug":project.slug,"name":project.name,"status":project.status.value,"instructions":project.instructions,"assets":assets,"asset_count":len(assets),"created_at":project.created_at.isoformat(),"updated_at":project.updated_at.isoformat()})
        except Exception as e:return bridge_error(str(e))
    def create_project(self,name:str,instructions:str="")->dict:
        ...
        try:
            slug=self._state.get_active_slug()
            if not slug:return bridge_error("No brand selected")
            if not name.strip():return bridge_error("Project name is required")
            project_slug=re.sub(r"[^a-z0-9]+","-",name.lower()).strip("-")
            if not project_slug:return bridge_error("Invalid project name")
            if load_project(slug,project_slug):return bridge_error(f"Project '{project_slug}' already exists")
            now=datetime.utcnow()
    ...
    def update_project(self,project_slug:str,name:str|None=None,instructions:str|None=None,status:str|None=None)->dict:
        ...
        try:
            ...
            if status is not None:
                ...
                else:return bridge_error(f"Invalid status: {status}")
            project.updated_at=datetime.utcnow();save_project(slug,project)
            return bridge_ok({"slug":project.slug,"name":project.name,"status":project.status.value})
        except Exception as e:return bridge_error(str(e))
    def delete_project(self,project_slug:str)->dict:
        ...
        try:
            ...
            if not deleted:return bridge_error(f"Project '{project_slug}' not found")
            return bridge_ok()
        except Exception as e:return bridge_error(str(e))
