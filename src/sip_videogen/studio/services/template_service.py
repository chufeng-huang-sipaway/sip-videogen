...
class TemplateService:
                    ...
                    "templates": [
                        {
                            ...
                            "default_strict": t.default_strict,
    ...
    def get_template(self, template_slug: str) -> dict:
        ...
        try:
            ...
            if not template:
                return bridge_error(f"Template '{template_slug}' not found")
            analysis_data = None
            if template.analysis:
                analysis_data = template.analysis.model_dump()
            return bridge_ok(
                {
                    "slug": template.slug,
                    "name": template.name,
                    ...
                    "primary_image": template.primary_image,
                    "default_strict": template.default_strict,
    ...
    def create_template(
        ...
        try:
            ...
            if not slug:
                return bridge_error("No brand selected")
            if not name.strip():
                return bridge_error("Template name is required")
            template_slug = re.sub(r"[^a-z0-9]+", "-", name.lower()).strip("-")
            if not template_slug:
                return bridge_error("Invalid template name")
            if load_template(slug, template_slug):
                return bridge_error(f"Template '{template_slug}' already exists")
            now = datetime.utcnow()
            ...
            if images:
                for img in images:
                    ...
                    try:
                        ...
                        add_template_image(slug, template_slug, filename, content)
                    except Exception as e:
                        logger.warning("Failed to add template image %s: %s", filename, e)
            # Run V2 analyzer on uploaded images
            template = load_template(slug, template_slug)
            if template and template.images:
                template_dir = get_template_dir(slug, template_slug)
                img_paths = [
                    template_dir / "images" / Path(p).name
                    for p in template.images
                    if (template_dir / "images" / Path(p).name).exists()
                ]
                if img_paths:
                    try:
    ...
    def update_template(
        ...
        try:
            ...
            if default_strict is not None:
                template.default_strict = default_strict
            template.updated_at = datetime.utcnow()
            save_template(slug, template)
            return bridge_ok(
                {
    ...
    def upload_template_image(self, template_slug: str, filename: str, data_base64: str) -> dict:
        ...
        try:
            ...
            brand_relative_path = add_template_image(slug, template_slug, filename, content)
    ...
    def set_primary_template_image(self, template_slug: str, filename: str) -> dict:
        ...
        try:
            ...
            if brand_relative_path not in template.images:
                return bridge_error(f"Image '{filename}' not found in template")
    ...
    def get_template_image_thumbnail(self, path: str) -> dict:
        ...
        return get_image_thumbnail(brand_dir, path, "templates")
    ...
    def get_template_image_full(self, path: str) -> dict:
        ...
        if err:
            return bridge_error(err)
        return get_image_full(brand_dir, path, "templates")
