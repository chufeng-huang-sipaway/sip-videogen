...
from ..utils.bridge_types import ALLOWED_IMAGE_EXTS,ALLOWED_TEXT_EXTS,bridge_ok,bridge_error
...
class BrandService:
    ...
    def _sync_brand_index(self)->None:
        ...
        try:
            ...
            index=load_index();changed=False;valid=[]
            for entry in index.brands:
                brand_dir=brands_dir/entry.slug
                if brand_dir.exists()and(brand_dir/"identity.json").exists():valid.append(entry)
            ...
            for item in brands_dir.iterdir():
                ...
                if item.name not in[e.slug for e in valid]:
                    ...
                    if summary:
                        ...
                        valid.append(entry);changed=True
            if changed:
                index.brands=valid
                ...
                save_index(index);logger.info("Index updated with %d brands",len(valid))
    ...
    def get_brand_info(self,slug:str|None=None)->dict:
        ...
        try:
            ...
            return bridge_ok({"slug":target,"name":summary.name,"tagline":summary.tagline,"category":summary.category})
        except Exception as e:return bridge_error(str(e))
    ...
    def update_brand_identity_section(self,section:str,data:dict)->dict:
        ...
        try:
            ...
            if section not in valid:return bridge_error(f"Invalid section: {section}. Must be one of: {', '.join(sorted(valid))}")
            try:
                if section=="core":identity.core=BrandCoreIdentity.model_validate(data)
                    ...
                    if not isinstance(data,dict):return bridge_error("constraints_avoid section must be an object with 'constraints' and 'avoid' arrays")
                    constraints=data.get("constraints",[]);avoid=data.get("avoid",[])
                    if not isinstance(constraints,list)or not isinstance(avoid,list):return bridge_error("'constraints' and 'avoid' must be arrays")
                    identity.constraints=constraints;identity.avoid=avoid
            except Exception as ve:return bridge_error(f"Invalid {section} data: {ve}")
            save_brand(identity)
    ...
    def regenerate_brand_identity(self,confirm:bool)->dict:
        ...
        try:
            ...
            if not docs_dir.exists()or not any(docs_dir.iterdir()):return bridge_error("No source documents found. Add documents to the brand's docs/ folder before regenerating.")
            concept_parts=[]
            ...
            if not concept_parts:return bridge_error("No readable documents found in docs/ folder.")
            concept="\n\n---\n\n".join(concept_parts)
            ...
            except Exception as be:return bridge_error(f"Failed to backup current identity: {be}")
    ...
    def restore_identity_backup(self,filename:str)->dict:
        ...
        try:
            ...
            try:restored=restore_brand_backup(slug,filename)
            except ValueError as e:return bridge_error(str(e))
            restored.slug=slug;save_brand(restored)
    ...
    def _build_concept(self,description:str,documents:list[dict])->tuple[str|None,str|None]:
        """Build concept string from description and document contents."""
        parts=[]
        if description.strip():parts.append(f"## Brand Description\n\n{description.strip()}");logger.debug("Added description to concept")
        for doc in documents:
            fn=doc.get("filename","unknown");data=doc.get("data","")
            try:
            ...
            except Exception as e:logger.error("Error reading %s: %s",fn,e)
        if not parts:return None,"Please provide a description or upload documents."
        concept="\n\n---\n\n".join(parts)
        ...
        if len(concept)>max_len:
            ...
            if description.strip():
                desc_part=f"## Brand Description\n\n{description.strip()}";remaining=max_len-len(desc_part)-100
                if remaining>500:doc_summary=concept[len(desc_part):][:remaining];concept=desc_part+"\n\n---\n\n"+doc_summary+"\n...[truncated]"
                else:concept=desc_part[:max_len]
            else:concept=concept[:max_len]+"\n...[truncated]"
    ...
    def create_brand_from_materials(self,description:str,images:list[dict],documents:list[dict])->dict:
        ...
        try:
            ...
            if err:logger.error("Concept error: %s",err);return bridge_error(err)
            #Run AI
            identity,err=self._run_brand_director(concept)
            if err:self._state.current_progress="";return bridge_error(err)
            #Save brand
        ...
        except ValueError as e:logger.error("ValueError: %s",e);self._state.current_progress="";return bridge_error(str(e))
        except Exception as e:
