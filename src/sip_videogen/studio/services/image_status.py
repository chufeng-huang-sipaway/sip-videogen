...
class ImageStatusService:
    ...
    def _load_status_data(self,brand_slug:str)->dict:
        ...
        if changed:self._save_status_data(brand_slug,data)
        return data
    ...
    def _ensure_unread_tracking_started_at(self,data:dict)->tuple[str,bool]:
        ...
        if isinstance(existing,str)and existing:return existing,False
        now=self._now_iso();data[UNREAD_TRACKING_STARTED_AT_KEY]=now
        return now,True
    def _migrate_legacy_viewed_at(self,data:dict,baseline_iso:str)->bool:
        ...
        if not isinstance(images,dict):return False
        changed=False
        for entry in images.values():
            ...
            if "viewedAt"not in entry:
                ...
                entry["viewedAt"]=baseline_iso;changed=True
        return changed
    def _migrate_legacy_status_fields(self,data:dict)->bool:
        ...
        for entry in images.values():
            if not isinstance(entry,dict):continue
            if "keptAt"not in entry:entry["keptAt"]=None;changed=True
            if "trashedAt"not in entry:entry["trashedAt"]=None;changed=True
        return changed
        ...
        try:
            ...
            if source_template_path and not source_template_path.startswith(("data:","http://","https://")):
                ...
                if not p.is_absolute():
                    ...
                    source_path=str(p)
            entry={"id":image_id,"status":"unsorted","originalPath":image_path,"currentPath":image_path,"prompt":prompt,"sourceTemplatePath":source_path,"timestamp":now,"viewedAt":None,"keptAt":None,"trashedAt":None}
    ...
    def mark_viewed(self,brand_slug:str,image_id:str)->dict:
        ...
        try:
            ...
            if not entry.get("viewedAt"):entry["viewedAt"]=self._now_iso();images[image_id]=entry;data["images"]=images;self._save_status_data(brand_slug,data)
            return bridge_ok(entry)
        except Exception as e:return bridge_error(str(e))
    ...
    def register_or_find(self,brand_slug:str,path:str,status:ImageStatus="unsorted")->dict:
        ...
        try:
            ...
            entry={"id":image_id,"status":status,"originalPath":path,"currentPath":path,"prompt":None,"sourceTemplatePath":None,"timestamp":now,"viewedAt":None,"keptAt":kept_at,"trashedAt":trashed_at}
            data["images"][image_id]=entry;self._save_status_data(brand_slug,data)
            return bridge_ok(entry)
        except Exception as e:return bridge_error(str(e))
    def backfill_from_folders(self,brand_slug:str)->dict:
        ...
        try:
            ...
            existing_paths={self._normalize_path(e.get("currentPath"))for e in data.get("images",{}).values()}
            added=[]
            ...
            for folder,status in folders_status:
                ...
                for fp in folder_path.iterdir():
                    ...
                    path_str=str(fp)
                    if self._normalize_path(path_str)in existing_paths:continue
                    ...
                    entry={"id":image_id,"status":status,"originalPath":path_str,"currentPath":path_str,"prompt":None,"sourceTemplatePath":None,"timestamp":ts,"viewedAt":viewed_at,"keptAt":kept_at,"trashedAt":trashed_at}
    ...
    def cleanup_old_trash(self,brand_slug:str,days:int=30)->dict:
        ...
        try:
            ...
            for image_id,entry in images.items():
                if not isinstance(entry,dict)or entry.get("status")!="trashed":continue
                trashed_dt=self._parse_iso(entry.get("trashedAt"))
                if not trashed_dt or trashed_dt>=cutoff:continue
