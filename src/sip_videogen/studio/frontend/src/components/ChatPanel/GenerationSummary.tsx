...
<div><span className="text-muted-foreground">Products:</span>{' '}{metadata.product_slugs.join(', ')} <span className="text-green-500">✓</span></div>)}
...
{metadata.validation_passed===false&&<span className="text-yellow-500 ml-1">⚠ warnings</span>}</div>)}
{metadata.validation_warning&&(<div className="text-yellow-600 dark:text-yellow-400 text-xs">⚠ {metadata.validation_warning}</div>)}
