...
if(!oT)return <div className="text-xs"><span className="text-green-600">+ {tr(fT,400)}</span></div>
...
return(<div className="text-xs space-y-2"><div><span className="text-muted-foreground">Original:</span><span className="ml-2">{tr(oT,200)}</span></div>{adds&&(<div className="text-green-600 dark:text-green-400 whitespace-pre-wrap"><span className="font-medium">+ Added:</span><div className="pl-4">{tr(adds,500)}</div></div>)}</div>)}
