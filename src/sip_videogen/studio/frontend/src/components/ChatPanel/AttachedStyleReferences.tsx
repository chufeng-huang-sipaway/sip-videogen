...
export function AttachedStyleReferences({styleReferences,attachedStyleReferences,onDetach,onToggleStrict}:AttachedStyleReferencesProps){
...
<span>Style References attached to chat</span>
</div>
<div className="flex flex-wrap gap-2">
{attachedStyleReferences.map(({style_reference_slug,strict})=>{
...
<button type="button"className={`p-1 rounded transition-colors ${strict?'text-brand-500 hover:text-brand-600 bg-brand-a10':'text-muted-foreground/60 hover:text-muted-foreground'}`}onClick={()=>onToggleStrict(style_reference_slug,!strict)}title={strict?'Strict mode ON - click to allow variation':'Loose mode - click to enforce strict style'}>
{strict?<Lock className="h-3 w-3"/>:<Unlock className="h-3 w-3"/>}
</button>
<button type="button"className="text-muted-foreground/60 hover:text-destructive transition-colors"onClick={()=>onDetach(style_reference_slug)}title="Remove from chat">
<X className="h-3 w-3"/>
</button>
</div>)})}
</div>
