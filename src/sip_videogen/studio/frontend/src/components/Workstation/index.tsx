...
return(<div className="flex-1 flex flex-col bg-secondary/20 dark:bg-secondary/10">{hasImages?(<>{/* Header with compare toggle */}<div className="flex items-center justify-end px-4 py-2 border-b border-border/50"><Button variant={isComparison?'secondary':'ghost'} size="sm" onClick={toggleComparison} className="gap-1.5 text-xs"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 17V7m0 10a2 2 0 01-2 2H5a2 2 0 01-2-2V7a2 2 0 012-2h2a2 2 0 012 2m0 10a2 2 0 002 2h2a2 2 0 002-2M9 7a2 2 0 012-2h2a2 2 0 012 2m0 10V7m0 10a2 2 0 002 2h2a2 2 0 002-2V7a2 2 0 00-2-2h-2a2 2 0 00-2 2"/></svg>{isComparison?'Single View':'Compare'}</Button></div>{/* Main content: comparison or swipe view */}{isComparison?(<ComparisonView/>):(<SwipeContainer onSwipeRight={handleKeep} onSwipeLeft={handleTrash} disabled={!currentImage}><ImageDisplay/></SwipeContainer>)}<ThumbnailStrip/></>):(<EmptyState/>)}</div>)}
