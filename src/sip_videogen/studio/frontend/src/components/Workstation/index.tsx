...
const{currentBatch,selectedIndex,viewMode,setViewMode,setCurrentBatch,setSelectedIndex,removeFromUnsorted,addToUnsorted,isTrashView,setIsTrashView}=useWorkstation()
...
const undoKeep=useCallback(async(image:GeneratedImage)=>{if(!activeBrand)return;try{await bridge.unkeepImage(image.id,activeBrand);addToUnsorted([image]);setCurrentBatch([image,...currentBatch]);setSelectedIndex(0);toast.success('Image restored to unsorted')}catch(e){console.error('Failed to undo keep:',e);toast.error('Failed to undo')}},[activeBrand,addToUnsorted,currentBatch,setCurrentBatch,setSelectedIndex])
//Undo trash action - restore to unsorted and add back to batch
const undoTrash=useCallback(async(image:GeneratedImage)=>{if(!activeBrand)return;try{await bridge.restoreImage(image.id,activeBrand);addToUnsorted([image]);setCurrentBatch([image,...currentBatch]);setSelectedIndex(0);toast.success('Image restored to unsorted')}catch(e){console.error('Failed to undo trash:',e);toast.error('Failed to undo')}},[activeBrand,addToUnsorted,currentBatch,setCurrentBatch,setSelectedIndex])
