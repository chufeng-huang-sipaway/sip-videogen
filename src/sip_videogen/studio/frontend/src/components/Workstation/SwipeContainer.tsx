...
return(<div ref={ref} className="relative flex-1 flex items-center justify-center select-none cursor-grab active:cursor-grabbing overflow-hidden" style={{background:bgColor,transition:'background 0.15s ease-out'}} onMouseDown={handleMouseDown} onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onMouseLeave={handleMouseLeave} onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}><div className="relative will-change-transform" style={{transform:`translateX(${offset}px) rotate(${rotation}deg)`,opacity:exitOpacity,transition:state.isDragging?'none':'all 0.3s cubic-bezier(0.4,0,0.2,1)',border:absOffset>10?`2px solid ${borderColor}`:'2px solid transparent',borderRadius:'0.5rem',padding:'0.25rem'}}>{children}{absOffset>30&&!state.exitDir&&(<div className="absolute inset-0 flex items-center justify-center pointer-events-none"><div className={`text-2xl font-bold uppercase tracking-wider px-4 py-2 rounded border-4 transition-opacity duration-150 ${dragOffset>0?'text-green-500 border-green-500 bg-green-500/20 rotate-[-15deg]':'text-red-500 border-red-500 bg-red-500/20 rotate-[15deg]'}`} style={{opacity:progress}}>{dragOffset>0?'Keep':'Trash'}</div></div>)}</div></div>)}
