...
import type { ConstraintsAvoidData, BrandIdentityFull } from '@/types/brand-identity'
 ...
 */
function deepCopyConstraintsAvoid(data: ConstraintsAvoidData): ConstraintsAvoidData {
  return {
...
export function ConstraintsAvoidSection({ data, onIdentityUpdate }: ConstraintsAvoidSectionProps) {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      const updatedIdentity = await bridge.updateBrandIdentitySection('constraints_avoid', editData)
  ...
  const subtitle =
    constraintsCount > 0 || avoidCount > 0
      ? `${constraintsCount} constraint${constraintsCount !== 1 ? 's' : ''}, ${avoidCount} to avoid`
      ...
      <div className="grid gap-4">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground mb-2">
            Rules and requirements the brand must always follow
          </p>
          <div className="mt-1 flex flex-wrap gap-2">
            {data.constraints.length > 0 ? (
              ...
              <span className="text-sm text-muted-foreground italic">No constraints defined</span>
            )}
          </div>
        </div>
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground mb-2">
            Content, messaging, or approaches the brand should never use
          </p>
          <div className="mt-1 flex flex-wrap gap-2">
            {data.avoid.length > 0 ? (
              ...
              <span className="text-sm text-muted-foreground italic">No items to avoid defined</span>
            )}
          </div>
        </div>
      </div>
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-4">
        ...
        {renderStringListEditor(
          'constraints',
          'Brand Constraints',
          'e.g., Always use eco-friendly messaging'
        )}
        ...
        {renderStringListEditor(
          'avoid',
          'Things to Avoid',
          'e.g., Never use aggressive sales language'
        )}
      </div>
