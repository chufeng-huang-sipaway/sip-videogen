//CreateStyleReferenceDialog for creating new style references
...
interface CreateStyleReferenceDialogProps{open:boolean;onOpenChange:(open:boolean)=>void;onCreated?:(slug:string)=>void}
export function CreateStyleReferenceDialog({open,onOpenChange,onCreated}:CreateStyleReferenceDialogProps){
...
//Limit to 2 images max for style references
const remaining=2-images.length
if(processed.length>remaining){
setUploadError(prev=>prev?prev+' Style references allow max 2 images.':'Style references allow max 2 images.')
...
if(images.length===0)throw new Error('Please add at least one reference image.')
const imageData=images.map(({file,base64})=>({filename:file.name,data:base64}))
const slug=await createStyleReference({name:name.trim(),description:description.trim(),images:imageData,defaultStrict})
await refresh();toast.success(`Style Reference "${name.trim()}" created`);onCreated?.(slug);onOpenChange(false)
...
return(<FormDialog open={open} onOpenChange={handleClose} title="Add New Style Reference" description="Create a style reference from reference images. AI will analyze the visual language." icon={<Layout className="h-5 w-5"/>} iconColor="text-brand-500" isLoading={isLoading} loadingMessage="Creating style reference..." error={combinedError} onClearError={()=>{clearError();setUploadError(null)}} footer={<>
...
<label htmlFor="style-ref-name" className="text-sm font-medium">Name <span className="text-destructive">*</span></label>
<Input id="style-ref-name" value={name} onChange={(e)=>setName(e.target.value)} placeholder="e.g., Hero Banner Style" autoFocus/>
</div>
{/*Description Input*/}
<div className="space-y-2">
<label htmlFor="style-ref-description" className="text-sm font-medium">Description</label>
<textarea id="style-ref-description" value={description} onChange={(e)=>setDescription(e.target.value)} placeholder="Describe the visual style (e.g., warm lighting, natural tones, lifestyle feel)" rows={2} className="w-full px-3 py-2 text-sm border rounded-md bg-transparent focus:outline-none focus:ring-2 focus:ring-brand-500 resize-none"/>
</div>
...
<DropzoneEmptyState><div className="flex flex-col items-center"><p className="text-sm mb-1">Drag & drop reference images</p><p className="text-xs text-muted-foreground">PNG, JPG, GIF, WebP</p></div></DropzoneEmptyState>
</Dropzone>
</div>
{/*Image Preview List*/}
...
{images.length===0&&(<p className="text-xs text-muted-foreground">Upload 1-2 reference images showing the desired visual style.</p>)}
</FormDialog>)}
