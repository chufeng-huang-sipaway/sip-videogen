...
return(<FormDialog open={open} onOpenChange={handleClose} title="Add New Template" description="Create a layout template from reference images. AI will analyze the composition." icon={<Layout className="h-5 w-5"/>} iconColor="text-indigo-500" isLoading={isLoading} loadingMessage="Creating template..." error={combinedError} onClearError={()=>{clearError();setUploadError(null)}} footer={<>
<Button variant="outline" onClick={handleClose} disabled={isLoading}>Cancel</Button>
<Button onClick={()=>execute()} disabled={isLoading||!name.trim()||images.length===0} className="bg-indigo-600 hover:bg-indigo-700">{isLoading?'Creating...':'Add Template'}</Button>
</>}>
{/*Name Input*/}
...
<textarea id="template-description" value={description} onChange={(e)=>setDescription(e.target.value)} placeholder="Describe the layout purpose (e.g., product hero, promotional banner, social post)" rows={2} className="w-full px-3 py-2 text-sm border rounded-md bg-transparent focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none"/>
</div>
