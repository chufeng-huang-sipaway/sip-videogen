...
export interface GeneratedImage{id:string;path:string;prompt?:string;sourceTemplatePath?:string;timestamp:string}
//Workstation state interface
export interface WorkstationState{currentBatch:GeneratedImage[];selectedIndex:number;viewMode:'single'|'comparison';comparisonSource:string|null;unsortedImages:GeneratedImage[]}
...
const clearCurrentBatch=useCallback(()=>{setCurrentBatchState([]);setSelectedIndexState(0)},[])
return(<WorkstationContext.Provider value={{currentBatch,selectedIndex,viewMode,comparisonSource,unsortedImages,setCurrentBatch,setSelectedIndex,setViewMode,setComparisonSource,addToUnsorted,removeFromUnsorted,clearCurrentBatch}}>{children}</WorkstationContext.Provider>)}
