...
from .bridge_types import ALLOWED_IMAGE_EXTS,MIME_TYPES,bridge_ok,bridge_error
...
def get_image_thumbnail(brand_dir:Path,path:str,path_prefix:str)->dict:
    ...
    """
    if not path.startswith(f"{path_prefix}/"):return bridge_error(f"Path must start with '{path_prefix}/'")
    ...
    if not resolved.is_file():return bridge_error("Path is not a file")
    ...
    except UnidentifiedImageError:return bridge_error("Cannot decode image file")
    except Image.DecompressionBombError:return bridge_error("Image too large")
    except OSError as e:return bridge_error(f"Failed to read image: {e}")
