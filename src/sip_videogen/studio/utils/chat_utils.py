...
async def process_attachments(attachments:list[dict]|None,brand_dir:Path,resolve_assets_fn:Callable,resolve_docs_fn:Callable)->list[tuple[str,str,Path,bool]]:
    """Process attachments and return (filename, rel_path, full_path, is_image) tuples."""
    saved:list[tuple[str,str,Path,bool]]=[]
    if not attachments:return saved
    upload_dir=brand_dir/"uploads";upload_dir.mkdir(parents=True,exist_ok=True)
    for idx,att in enumerate(attachments):
        ...
        rel_path:str|None=None;full_path:Path|None=None
        data_b64=att.get("data");ref_path=att.get("path")
        if data_b64:
            try:
                content=base64.b64decode(data_b64)
                if len(content)>MAX_ATTACHMENT_BYTES:continue
                suffix=Path(safe_name).suffix
                allowed=ALLOWED_IMAGE_EXTS|ALLOWED_TEXT_EXTS
                if suffix and suffix.lower()not in allowed:continue
    ...
    return saved
async def analyze_and_format_attachments(saved_attachments:list[tuple[str,str,Path,bool]],brand_dir:Path)->str:
        ...
        if is_image and full_path.suffix.lower()not in{".svg",".gif"}:
            try:
                ...
                if analysis is not None and rel_path.startswith("uploads/"):
                    ...
                    except Exception as e:logger.debug("Failed to write upload analysis cache for %s: %s",full_path,e)
            except Exception as e:
                logger.warning(f"Image analysis failed for {filename}: {e}")
                analyses.append((filename,rel_path,None))
        else:analyses.append((filename,rel_path,None))
...
def encode_new_images(paths:list[str],get_image_metadata_fn:Callable)->list[dict]:
    ...
    for img_path in paths[:4]:
        try:
            path=Path(img_path)
            if not path.exists()or path.suffix.lower()not in{".png",".jpg",".jpeg",".webp"}:continue
