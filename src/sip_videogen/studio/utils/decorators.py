...
def require_brand(param_name:str="brand_slug"):
    ...
    def decorator(method):
        ...
        if param_name not in params and not has_var_kw:
            raise TypeError(f"@require_brand: method {method.__name__} has no '{param_name}' parameter")
        #Find VAR_KEYWORD param name if exists
        var_kw_name=next((n for n,p in params.items()if p.kind==inspect.Parameter.VAR_KEYWORD),None)
        ...
        def sync_wrapper(self,*args,**kwargs):
            ...
            if not slug:
                ...
                if param_name not in params and var_kw_name:
                    if var_kw_name not in bound.arguments:bound.arguments[var_kw_name]={}
                    bound.arguments[var_kw_name][param_name]=slug
                else:bound.arguments[param_name]=slug
