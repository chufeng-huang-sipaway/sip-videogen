...
def load_api_keys_from_config()->None:
    ...
    if keys.get("firecrawl")and not os.environ.get("FIRECRAWL_API_KEY"):os.environ["FIRECRAWL_API_KEY"]=keys["firecrawl"]
def save_api_keys(openai_key:str,gemini_key:str,firecrawl_key:str="")->None:
    ...
    c=_load_config();ak=c.get("api_keys",{})
    ...
    #Update environment
    if o:os.environ["OPENAI_API_KEY"]=o
    if g:os.environ["GEMINI_API_KEY"]=g
    if f:os.environ["FIRECRAWL_API_KEY"]=f
    c["api_keys"]={"openai":o,"gemini":g,"firecrawl":f}
    _save_config(c)
def check_api_keys()->dict[str,bool]:
    """Check if required API keys are configured."""
    o=bool(os.environ.get("OPENAI_API_KEY"));g=bool(os.environ.get("GEMINI_API_KEY"));f=bool(os.environ.get("FIRECRAWL_API_KEY"))
