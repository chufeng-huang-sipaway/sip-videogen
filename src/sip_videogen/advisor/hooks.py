...
class AdvisorHooks(RunHooks):
    ...
    def __init__(self,callback:ProgressCallback|None=None):
        ...
        self._tool_descriptions={"generate_image":"Generating image with Gemini","read_file":"Reading file","write_file":"Writing file","list_files":"Listing directory contents","load_brand":"Loading brand context","propose_choices":"Presenting options to the user","propose_images":"Showing images for selection","update_memory":"Recording a preference"}
    def _report(self,progress:AdvisorProgress)->None:
        """Report progress to callback if set."""
        if self.callback:self.callback(progress)
    ...
    async def on_tool_start(self,context:RunContextWrapper,agent:Agent,tool:Tool)->None:
        ...
        if tool_name=="report_thinking":return
        description=self._tool_descriptions.get(tool_name,f"Running {tool_name}")
        self._report(AdvisorProgress(event_type="tool_start",message=f"Using {tool_name}",detail=description))
    async def on_tool_end(self,context:RunContextWrapper,agent:Agent,tool:Tool,result:str)->None:
        """Called when a tool call completes."""
        from sip_videogen.advisor.tools import get_pending_interaction,get_pending_memory_update,parse_thinking_step_result
        tool_name=tool.name
        ...
        if tool_name=="report_thinking":
            ...
            if step_data:self._report(AdvisorProgress(event_type="thinking_step",message=step_data["step"],detail=step_data["detail"]))
            return  #Skip normal tool_end reporting
        if tool_name in("propose_choices","propose_images"):
            interaction=get_pending_interaction()
            if interaction:self.captured_interaction=interaction
        if tool_name=="update_memory":
            mem_update=get_pending_memory_update()
            if mem_update:self.captured_memory_update=mem_update
        result_preview=str(result)[:100]+"..." if len(str(result))>100 else str(result)
        self._report(AdvisorProgress(event_type="tool_end",message=f"{tool_name} completed",detail=result_preview))
