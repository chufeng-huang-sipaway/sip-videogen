...
from sip_videogen.brands.storage import get_brand_dir,load_brand
if TYPE_CHECKING:
...
def format_brand_context(slug:str,identity:"BrandIdentityFull")->str:
    ...
    """
    p=["## Current Brand Context","",f"**Brand**: {identity.core.name}",f"**Tagline**: {identity.core.tagline}",f"**Category**: {identity.positioning.market_category}",""]
    #Visual identity summary
    if identity.visual.primary_colors:
        c=", ".join(f"{c.name} ({c.hex})" for c in identity.visual.primary_colors[:3])
        p.append(f"**Colors**: {c}")
    if identity.visual.style_keywords:p.append(f"**Style**: {', '.join(identity.visual.style_keywords[:5])}")
    #Voice summary
    if identity.voice.tone_attributes:p.append(f"**Tone**: {', '.join(identity.voice.tone_attributes[:3])}")
    #Audience summary
    ...
    p.append("")
    p.append("Use `load_brand()` for a quick summary, or `load_brand(detail_level='full')` for complete context. Use `list_files()` to browse assets.")
    return "\n".join(p)
def build_system_prompt(brand_slug:str|None=None)->str:
    ...
    if brand_slug:
        ...
        if ident:brand_sec=format_brand_context(brand_slug,ident)
        ...
        if mp.exists():
            try:
                ...
                if mem:
                    ...
                    for k,d in mem.items():ml.append(f"- **{k}**: {d.get('value','')}")
                    mem_sec="\n".join(ml)
