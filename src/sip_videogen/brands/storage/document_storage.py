...
from sip_videogen.constants import ALLOWED_IMAGE_EXTS,ALLOWED_TEXT_EXTS,ALLOWED_VIDEO_EXTS,ASSET_CATEGORIES
...
def list_documents(brand_slug:str)->list[dict]:
    ...
    for p in sorted(dd.rglob("*")):
        if not p.is_file()or p.name.startswith(".")or p.suffix.lower()not in ALLOWED_TEXT_EXTS:continue
...
def delete_document(brand_slug:str,relative_path:str)->tuple[bool,str|None]:
    ...
    """
    dd=get_docs_dir(brand_slug)
    if not dd.exists():return False,"Docs directory not found"
    resolved,err=_safe_resolve_in_dir(dd,relative_path)
...
def rename_document(brand_slug:str,relative_path:str,new_name:str)->tuple[str|None,str|None]:
    ...
    """
    dd=get_docs_dir(brand_slug)
    if not dd.exists():return None,"Docs directory not found"
    resolved,err=_safe_resolve_in_dir(dd,relative_path)
    if err:return None,err
    ...
    if np.exists():return None,f"File already exists: {new_name}"
    resolved.rename(np)
