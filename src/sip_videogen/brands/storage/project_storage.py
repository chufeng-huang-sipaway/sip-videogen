...
from sip_videogen.exceptions import BrandNotFoundError,ProjectNotFoundError,DuplicateEntityError
...
def load_project_index(brand_slug:str)->ProjectIndex:
    ...
    if ip.exists():
        try:
            ...
            logger.debug("Loaded project index for %s with %d projects",brand_slug,len(idx.projects))
            return idx
        except json.JSONDecodeError as e:logger.warning("Invalid JSON in project index for %s: %s",brand_slug,e)
        except Exception as e:logger.warning("Failed to load project index for %s: %s",brand_slug,e)
...
def count_project_assets(brand_slug:str,project_slug:str)->int:
    ...
    for ad in ads:
        ...
        for fp in ad.iterdir():
            if fp.is_file()and fp.suffix.lower()in ae and fp.name.startswith(pf):cnt+=1
    return cnt
...
def create_project(brand_slug:str,project:ProjectFull)->ProjectSummary:
    ...
    if pd.exists():raise DuplicateEntityError(f"Project '{project.slug}' already exists in brand '{brand_slug}'")
    #Create directory
...
def save_project(brand_slug:str,project:ProjectFull)->ProjectSummary:
    ...
    """
    pd=get_project_dir(brand_slug,project.slug)
    if not pd.exists():return create_project(brand_slug,project)
    #Update timestamp
...
def get_active_project(brand_slug:str)->str|None:
    ...
    idx=load_project_index(brand_slug)
    logger.debug("get_active_project(%s) -> %s (from index with %d projects)",brand_slug,idx.active_project,len(idx.projects))
