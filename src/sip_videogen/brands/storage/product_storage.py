...
from sip_videogen.exceptions import BrandNotFoundError,ProductNotFoundError,DuplicateEntityError
...
def load_product_index(brand_slug:str)->ProductIndex:
    ...
    if ip.exists():
        try:
            ...
            logger.debug("Loaded product index for %s with %d products",brand_slug,len(idx.products))
            return idx
        except json.JSONDecodeError as e:logger.warning("Invalid JSON in product index for %s: %s",brand_slug,e)
        except Exception as e:logger.warning("Failed to load product index for %s: %s",brand_slug,e)
...
def create_product(brand_slug:str,product:ProductFull)->ProductSummary:
    ...
    if pd.exists():raise DuplicateEntityError(f"Product '{product.slug}' already exists in brand '{brand_slug}'")
...
def save_product(brand_slug:str,product:ProductFull)->ProductSummary:
    ...
    """
    pd=get_product_dir(brand_slug,product.slug)
    if not pd.exists():return create_product(brand_slug,product)
    #Update timestamp
...
def add_product_image(brand_slug:str,product_slug:str,filename:str,data:bytes)->str:
    ...
    """
    pd=get_product_dir(brand_slug,product_slug)
    if not pd.exists():raise ProductNotFoundError(f"Product '{product_slug}' not found in brand '{brand_slug}'")
    imd=pd/"images"
    ...
    if prod:
        if br not in prod.images:
            ...
            if not prod.primary_image:prod.primary_image=br
...
def delete_product_image(brand_slug:str,product_slug:str,filename:str)->bool:
    ...
    if prod:
        if br in prod.images:
            ...
            if prod.primary_image==br:prod.primary_image=prod.images[0]if prod.images else""
            save_product(brand_slug,prod)
...
def set_primary_product_image(brand_slug:str,product_slug:str,brand_relative_path:str)->bool:
    ...
    if brand_relative_path not in prod.images:return False
    prod.primary_image=brand_relative_path
    save_product(brand_slug,prod)
    logger.info("Set primary image for product %s/%s to %s",brand_slug,product_slug,brand_relative_path)
    return True
