...
def validate_slug(slug:str)->None:
    ...
    if not slug or len(slug)>100:raise ValueError("Slug empty or too long")
    if".."in slug or"/"in slug or"\\"in slug:raise ValueError("Invalid slug: path traversal")
    #Note: Allow underscores and uppercase to match existing data
    if not re.match(r"^[a-zA-Z0-9][a-zA-Z0-9_-]*$",slug):raise ValueError("Invalid slug format")
def safe_resolve_path(base:Path,relative:str)->Path:
    ...
    try:
        ...
        if common!=base_resolved:raise ValueError("Path traversal detected")
    except ValueError:
