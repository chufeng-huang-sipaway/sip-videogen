...
from ..models import BrandIdentityFull,BrandIndexEntry,BrandSummary,TemplateIndex
from .base import get_brand_dir,get_brands_dir
...
def create_brand(identity:BrandIdentityFull)->BrandSummary:
    ...
    if bd.exists():raise DuplicateEntityError(f"Brand '{identity.slug}' already exists")
    #Create directory structure
    bd.mkdir(parents=True,exist_ok=True)
    (bd/"assets").mkdir(exist_ok=True)
    for cat in ASSET_CATEGORIES:
        if cat!="generated":(bd/"assets"/cat).mkdir(exist_ok=True)
    (bd/"history").mkdir(exist_ok=True)
    #Initialize templates directory with empty index
    td=bd/"templates"
    td.mkdir(exist_ok=True)
    write_atomically(td/"index.json",TemplateIndex().model_dump_json(indent=2))
    ...
    #Update index
    idx=load_index()
    entry=BrandIndexEntry(slug=identity.slug,name=identity.core.name,category=identity.positioning.market_category,created_at=identity.created_at,updated_at=identity.updated_at)
    idx.add_brand(entry)
    save_index(idx)
...
def save_brand(identity:BrandIdentityFull)->BrandSummary:
    ...
    """
    bd=get_brand_dir(identity.slug)
    if not bd.exists():return create_brand(identity)
    #Update timestamp
        ...
        for cd in ad.iterdir():
            if cd.is_dir():
                for fp in cd.iterdir():
                    if fp.suffix.lower()in ALLOWED_IMAGE_EXTS:cnt+=1
...
def backup_brand_identity(slug:str)->str:
    ...
    bd=get_brand_dir(slug)
    ip=bd/"identity_full.json"
    if not bd.exists():raise BrandNotFoundError(f"Brand '{slug}' not found")
    if not ip.exists():raise BrandNotFoundError(f"Identity file not found for brand '{slug}'")
    ...
    for fp in hd.iterdir():
        if fp.is_file()and fp.name.startswith("identity_full_"):
            ...
            if len(parts)>=4:
                ...
                backups.append({"filename":fp.name,"timestamp":its,"size_bytes":fp.stat().st_size})
...
def restore_brand_backup(slug:str,filename:str)->BrandIdentityFull:
    ...
    if"/"in filename or"\\"in filename:raise StorageError("Invalid filename: path separators not allowed")
    #Validate filename format
    if not filename.endswith(".json"):raise StorageError("Invalid filename: must end with .json")
    if not filename.startswith("identity_full_"):raise StorageError("Invalid filename: must start with 'identity_full_'")
    #Verify file exists in history folder
    hd=bd/"history"
    bp=hd/filename
    if not bp.exists():raise StorageError(f"Backup '{filename}' not found for brand '{slug}'")
    #Ensure the resolved path is still within history/
    try:bp.resolve().relative_to(hd.resolve())
    except ValueError:raise StorageError("Invalid filename: path traversal detected")
    #Load and parse the backup
    try:
        data=json.loads(bp.read_text())
        identity=BrandIdentityFull.model_validate(data)
    except json.JSONDecodeError as e:raise StorageError(f"Invalid JSON in backup file: {e}")
    except Exception as e:raise StorageError(f"Failed to parse backup file: {e}")
    #Enforce slug stability
    if identity.slug!=slug:
        logger.warning("Backup slug '%s' differs from current brand '%s', forcing current slug",identity.slug,slug)
        identity.slug=slug
...
def set_active_brand(slug:str|None)->None:
    ...
    """
    idx=load_index()
    if slug and not idx.get_brand(slug):raise BrandNotFoundError(f"Brand '{slug}' not found")
