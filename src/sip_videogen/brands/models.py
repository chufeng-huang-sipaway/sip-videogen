...
class PackagingTextElement(BaseModel):
    ...
    notes: str = Field(default="", description="Disambiguation notes, e.g. 'letter O not zero', 'lowercase L not one'")
    role: str = Field(default="other", description="brand_name|product_name|tagline|instructions|legal|decorative|other")
    typography: str = Field(default="", description="serif|sans-serif|script|decorative|handwritten|geometric|monospace")
...
class PackagingTextDescription(BaseModel):
    ...
    generated_at: datetime | None = Field(default=None, description="When AI analysis was performed")
...
class ProductSummary(BaseModel):
    ...
    primary_image: str = Field(default="", description="Brand-relative path to primary image, e.g., 'products/night-cream/images/main.png'")
    ...
    created_at: datetime = Field(default_factory=datetime.utcnow, description="When the product was created")
    updated_at: datetime = Field(default_factory=datetime.utcnow, description="When the product was last updated")
...
class ProductFull(BaseModel):
    ...
    description: str = Field(description="English product description (i18n deferred)")
    images: List[str] = Field(default_factory=list, description="Brand-relative paths to all product images")
    primary_image: str = Field(default="", description="Brand-relative path to primary image (must be in images list)")
    attributes: List[ProductAttribute] = Field(default_factory=list, description="Product attributes (dimensions, materials, etc.)")
    packaging_text: PackagingTextDescription | None = Field(default=None, description="Structured description of packaging text")
    ...
    def to_summary(self) -> ProductSummary:
        ...
        return ProductSummary(slug=self.slug, name=self.name, description=self.description,
            primary_image=self.primary_image, attribute_count=len(self.attributes),
