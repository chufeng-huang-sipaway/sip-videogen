...
Use sip_videogen.advisor.tools instead.
...
def __getattr__(name):
    ...
    if name in("browse_brand_assets","fetch_brand_detail"):
        if name not in _cache:
            from sip_videogen.advisor import tools as advisor_tools
...
def set_brand_context(slug:str|None)->None:
    ...
    warnings.warn("set_brand_context() is deprecated. Use storage.set_active_brand().",DeprecationWarning,stacklevel=2)
    logger.warning("DEPRECATED: set_brand_context() called with slug=%s. Delegating to storage.set_active_brand().",slug)
    from sip_videogen.brands.storage import set_active_brand
    set_active_brand(slug)
def get_brand_context()->str|None:
    """DEPRECATED: Use get_active_brand() from sip_videogen.brands.storage instead."""
    warnings.warn("get_brand_context() deprecated. Use storage.get_active_brand().",DeprecationWarning,stacklevel=2)
