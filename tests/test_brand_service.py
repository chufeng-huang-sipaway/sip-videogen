...
from sip_videogen.brands.models import BrandIdentityFull,BrandCoreIdentity,VisualIdentity,VoiceGuidelines,AudienceProfile,CompetitivePositioning,BrandIndexEntry,ColorDefinition
#=============================================================================
#Fixtures
#=============================================================================
@pytest.fixture
...
def sample_identity()->BrandIdentityFull:
    ...
    return BrandIdentityFull(
        slug="test-brand",
        core=BrandCoreIdentity(name="Test Brand",tagline="Test tagline",mission="Test mission",values=["Quality"]),
        visual=VisualIdentity(primary_colors=[ColorDefinition(hex="#000000",name="Black",usage="Primary")],typography_style="Modern",image_style="Clean"),
        voice=VoiceGuidelines(personality="Friendly and professional",tone_attributes=["Professional","Warm"]),
        audience=AudienceProfile(primary_summary="Adults",pain_points=["Time"]),
        positioning=CompetitivePositioning(market_category="Tech",unique_value_proposition="Best",primary_competitors=[]),
        constraints=[],avoid=[],
...
def brands_dir(tmp_path,monkeypatch)->Path:
    ...
    monkeypatch.setattr("sip_videogen.studio.services.brand_service.get_brand_dir",lambda s:bd/s)
    return bd
#=============================================================================
#get_brands tests
#=============================================================================
class TestGetBrands:
    ...
    def test_returns_empty_list_when_no_brands(self,service,brands_dir):
        ...
        with patch("sip_videogen.studio.services.brand_service.list_brands",return_value=[]):
            with patch("sip_videogen.studio.services.brand_service.get_active_brand",return_value=None):
    ...
    def test_returns_brands_list(self,service,brands_dir):
        ...
        entries=[
            BrandIndexEntry(slug="brand-a",name="Brand A",category="Tech",created_at=datetime.utcnow(),updated_at=datetime.utcnow()),
            BrandIndexEntry(slug="brand-b",name="Brand B",category="Food",created_at=datetime.utcnow(),updated_at=datetime.utcnow()),
        ]
        with patch("sip_videogen.studio.services.brand_service.list_brands",return_value=entries):
            with patch("sip_videogen.studio.services.brand_service.get_active_brand",return_value="brand-a"):
                result=service.get_brands()
        assert result["success"]==True
    ...
    def test_handles_exception(self,service):
        ...
        with patch("sip_videogen.studio.services.brand_service.list_brands",side_effect=Exception("DB error")):
            result=service.get_brands()
...
class TestSetBrand:
    ...
    def test_sets_active_brand(self,service,state):
        ...
        entries=[BrandIndexEntry(slug="my-brand",name="My Brand",category="Tech",created_at=datetime.utcnow(),updated_at=datetime.utcnow())]
        with patch("sip_videogen.studio.services.brand_service.list_brands",return_value=entries):
    ...
    def test_preserves_existing_advisor(self,service,state):
        ...
        entries=[BrandIndexEntry(slug="new-brand",name="New",category="Tech",created_at=datetime.utcnow(),updated_at=datetime.utcnow())]
        with patch("sip_videogen.studio.services.brand_service.list_brands",return_value=entries):
...
class TestGetBrandInfo:
    ...
    def test_returns_brand_info(self,service):
        ...
        with patch("sip_videogen.studio.services.brand_service.get_active_brand",return_value="test"):
            with patch("sip_videogen.studio.services.brand_service.load_brand_summary",return_value=mock_summary):
    ...
    def test_error_when_brand_not_found(self,service):
        ...
        with patch("sip_videogen.studio.services.brand_service.get_active_brand",return_value="missing"):
            with patch("sip_videogen.studio.services.brand_service.load_brand_summary",return_value=None):
                result=service.get_brand_info()
        assert result["success"]==False
        assert "not found"in result["error"]
#=============================================================================
...
class TestGetBrandIdentity:
    ...
    def test_returns_full_identity(self,service,sample_identity):
        ...
        with patch("sip_videogen.studio.services.brand_service.get_active_brand",return_value="test-brand"):
            with patch("sip_videogen.studio.services.brand_service.load_brand",return_value=sample_identity):
...
class TestUpdateBrandIdentitySection:
        ...
        with patch("sip_videogen.studio.services.brand_service.get_active_brand",return_value="test"):
            with patch("sip_videogen.studio.services.brand_service.load_brand",return_value=sample_identity):
                with patch("sip_videogen.studio.services.brand_service.save_brand")as mock_save:
                    result=service.update_brand_identity_section("core",{"name":"New Name","tagline":"New tag","mission":"New mission","values":["New value"]})
        assert result["success"]==True
        ...
        with patch("sip_videogen.studio.services.brand_service.get_active_brand",return_value="test"):
            with patch("sip_videogen.studio.services.brand_service.load_brand",return_value=sample_identity):
                with patch("sip_videogen.studio.services.brand_service.save_brand"):
                    result=service.update_brand_identity_section("visual",{"primary_colors":[{"hex":"#FF0000","name":"Red","usage":"Primary"}],"typography_style":"Bold","image_style":"Vibrant"})
        assert result["success"]==True
    def test_updates_constraints_avoid(self,service,sample_identity):
        ...
        with patch("sip_videogen.studio.services.brand_service.get_active_brand",return_value="test"):
            with patch("sip_videogen.studio.services.brand_service.load_brand",return_value=sample_identity):
                with patch("sip_videogen.studio.services.brand_service.save_brand")as mock_save:
                    result=service.update_brand_identity_section("constraints_avoid",{"constraints":["Must be blue"],"avoid":["Red colors"]})
        assert result["success"]==True
        saved=mock_save.call_args[0][0]
    ...
    def test_refreshes_advisor_after_update(self,service,sample_identity,state):
        ...
        with patch("sip_videogen.studio.services.brand_service.get_active_brand",return_value="test"):
            with patch("sip_videogen.studio.services.brand_service.load_brand",return_value=sample_identity):
                with patch("sip_videogen.studio.services.brand_service.save_brand"):
                    service.update_brand_identity_section("core",{"name":"Updated","tagline":"t","mission":"m","values":[]})
...
class TestDeleteBrand:
    ...
    def test_deletes_brand(self,service):
        ...
        entries=[BrandIndexEntry(slug="to-delete",name="Delete Me",category="Tech",created_at=datetime.utcnow(),updated_at=datetime.utcnow())]
        with patch("sip_videogen.studio.services.brand_service.list_brands",return_value=entries):
            with patch("sip_videogen.studio.services.brand_service.get_active_brand",return_value=None):
                with patch("sip_videogen.studio.services.brand_service.storage_delete_brand",return_value=True):
    ...
    def test_clears_advisor_when_deleting_active(self,service,state):
        ...
        entries=[BrandIndexEntry(slug="active-brand",name="Active",category="Tech",created_at=datetime.utcnow(),updated_at=datetime.utcnow())]
        with patch("sip_videogen.studio.services.brand_service.list_brands",return_value=entries):
            with patch("sip_videogen.studio.services.brand_service.get_active_brand",return_value="active-brand"):
                with patch("sip_videogen.studio.services.brand_service.set_active_brand"):
                    with patch("sip_videogen.studio.services.brand_service.storage_delete_brand",return_value=True):
...
class TestListIdentityBackups:
    ...
    def test_returns_backup_list(self,service):
        ...
        with patch("sip_videogen.studio.services.brand_service.get_active_brand",return_value="test"):
            with patch("sip_videogen.studio.services.brand_service.list_brand_backups",return_value=backups):
...
class TestRestoreIdentityBackup:
    ...
    def test_restores_backup(self,service,sample_identity,state):
        ...
        with patch("sip_videogen.studio.services.brand_service.get_active_brand",return_value="test"):
            with patch("sip_videogen.studio.services.brand_service.restore_brand_backup",return_value=sample_identity):
                with patch("sip_videogen.studio.services.brand_service.save_brand"):
