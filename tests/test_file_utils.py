...
class TestWriteAtomically:
    ...
    def test_cleans_up_temp_on_failure(self,tmp_path:Path,monkeypatch):
        ...
        fp=tmp_path/"test.json"
        def fail_fsync(fd):raise OSError("fsync failed")
        monkeypatch.setattr(os,"fsync",fail_fsync)
        with pytest.raises(OSError):write_atomically(fp,"content")
    ...
    def test_no_partial_writes(self,tmp_path:Path):
        ...
        with pytest.raises((OSError,IsADirectoryError)):write_atomically(fp,'{"new":"data"}')
