...
class TestSkipReview:
    ...
    @pytest.mark.asyncio
    async def test_skip_review_returns_unreviewed_status(
        self, sample_shared_element: SharedElement, tmp_path: Path, mock_generator
    ):
    ...
    @pytest.mark.asyncio
    async def test_skip_review_bypasses_reviewer(
        self, sample_shared_element: SharedElement, tmp_path: Path, mock_generator
    ):
    ...
    @pytest.mark.asyncio
    async def test_skip_review_handles_generation_error(
    ...
    @pytest.mark.asyncio
    async def test_parallel_skip_review(
        self,
        ...
        with patch("sip_studio.agents.tools.image_production.ImageGenerator") as MockGen:
            ...
            async def mock_gen(element, output_dir, aspect_ratio=None):
                ...
                return GeneratedAsset(
                    ...
                    local_path=f"/tmp/test_{call_count}.png",
                )
    ...
    @pytest.mark.asyncio
    async def test_default_skip_review_is_false(
        self, sample_shared_element: SharedElement, tmp_path: Path
    ):
    ...
    def test_all_statuses_are_valid(self):
        ...
        for status in ["success", "fallback", "failed", "unreviewed"]:
            result = ImageGenerationResult(
                element_id="test",
                status=status,
                local_path="/tmp/test.png" if status != "failed" else "",
                attempts=[],
                final_prompt="test",
            )
...
class TestVariantsGeneration:
    ...
    @pytest.mark.asyncio
    async def test_generate_with_variants_accepts_first(
    ...
    @pytest.mark.asyncio
    async def test_generate_with_variants_rejects_then_accepts(
        self, sample_shared_element: SharedElement, tmp_path: Path, mock_generator_variants
    ):
    ...
    @pytest.mark.asyncio
    async def test_generate_with_variants_all_rejected_uses_fallback(
        self, sample_shared_element: SharedElement, tmp_path: Path, mock_generator_variants
    ):
    ...
    @pytest.mark.asyncio
    async def test_generate_with_variants_cleans_up_unused(
        self, sample_shared_element: SharedElement, tmp_path: Path, mock_generator_variants
    ):
    ...
    @pytest.mark.asyncio
    async def test_generate_with_variants_skip_review_uses_first(
        self, sample_shared_element: SharedElement, tmp_path: Path
    ):
        ...
        with patch("sip_studio.agents.tools.image_production.ImageGenerator") as MockGen:
            mock_asset = GeneratedAsset(
                asset_type=AssetType.REFERENCE_IMAGE,
                element_id="test",
                local_path=str(tmp_path / "test.png"),
            )
            instance = MockGen.return_value
            instance.generate_reference_image = AsyncMock(return_value=mock_asset)
            instance._get_aspect_ratio_for_element = MagicMock(return_value="1:1")
            manager = ImageProductionManager(gemini_api_key="test-key", output_dir=tmp_path)
            manager.generator = instance
            result = await manager.generate_with_variants(
                sample_shared_element, num_variants=3, skip_review=True
            )
            assert result.status == "unreviewed"
    ...
    @pytest.mark.asyncio
    async def test_parallel_with_variants(
        self,
        sample_shared_element: SharedElement,
        sample_environment_element: SharedElement,
        tmp_path: Path,
    ...
    @pytest.mark.asyncio
    async def test_variants_generation_fails_gracefully(
        self, sample_shared_element: SharedElement, tmp_path: Path
    ):
